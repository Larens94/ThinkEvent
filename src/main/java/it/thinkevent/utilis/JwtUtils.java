...
public class JwtUtils {
    ...
    public static String getJwtFromHttpRequest(HttpServletRequest request){
        String jwt = null;
        if(request.getHeader("jwt") != null){ //prendiamo nell'header il jwt e verifico se esiste.
            jwt = request.getHeader("jwt");     //il token è presente
        }else if(request.getCookies() != null){
            Cookie[] cookies = request.getCookies();  //lo cerco anche nei cookie il jwt e verifico se esiste.
            for (Cookie cookie : cookies) {
                if (cookie.getName().equals("jwt")) {
                    jwt = cookie.getValue(); //il token è presente
                }
            }
        }
        return jwt;
